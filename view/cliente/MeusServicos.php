<?php
// ...apenas comentários ou variáveis vindas do controller...
// Exemplo: $meus_servicos, $tipos_servico, $page, $per_page, $total_servicos, $servicos_paginados
?>
<!DOCTYPE html>
<html lang="pt-br">
...
<body>
        ...
        <?php else: ?>
            <div class="row vista-cards" id="servicosContainer">
                <?php foreach ($servicos_paginados as $item): ?>
                    <?php require __DIR__ . '/../components/ComponentServicoCard.php'; ?>
                <?php endforeach; ?>
            </div>
            <!-- Paginação -->
            <nav>
    ...
    <?php require_once __DIR__ . '/modals/modalPerfilCliente.php'; ?>
            ...
            <div class="modal-content">
                ...
                <form id="recusaForm">
                    <div class="modal-body">
                        ...
                        <p>Tem certeza que deseja recusar esta proposta?</p>
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle me-2"></i>
                            O prestador será notificado sobre a recusa desta proposta.
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
            ...
            fetch('gerenciar-proposta.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Opcional: recarregar a página para atualizar tudo
                        window.location.reload();
                    } else {
                        alert('Erro: ' + data.message);
                    }
            ...
            fetch('gerenciar-proposta.php', {
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        // Remover toast após ocultado
                        toast.addEventListener('hidden.bs.toast', () => {
                            document.body.removeChild(toast);
                        });

                        // Recarregar após 2 segundos para garantir atualização completa
                        setTimeout(() => {
                            window.location.reload();
                        }, 2000);
                    } else {
